...
the len of the input should be equal to 10
only a digit, hyphen and 'X' are allowed
...
func IsValidISBN(isbn string) bool {
	...
	if unicode.IsDigit(rune(isbn[9])) {
		sum += int(isbn[9] - '0')
	} else if rune(isbn[9]) == rune('X') {
		sum += 10
	} else {
		return false
	}
	...
	for count := 2; count <= 10; count++ {
		if !unicode.IsDigit(rune(isbn[l-count])) {
			return false
		}
		sum += int(isbn[l-count]-'0') * count
	}
	//d[8]*2
	// d[0]*10
	return sum%11 == 0
